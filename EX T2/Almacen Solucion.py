...
class Articulo:
    ...
    def __repr__(self):
        return f'Articulo({repr(self.get_nombre())}),{self.get_cantidad()})'
...
class Gestion_articulo:
    ...
    def alta_articulo(nombre):
        for articulo in Gestion_articulo.__lista_articulos:
            if articulo.get_nombre() == nombre:
                raise ValueError("Articulo ya dado de alta")
    ...
    def agregar_cantidad_articulo(nombre, cantidad):
        for articulo in Gestion_articulo.__lista_articulos:
            if articulo.get_nombre() == nombre:
                ...
                return
        raise ValueError("Articulo no dado de alta")
    ...
    def imprimir_articulos():
        ...
        for articulo in Gestion_articulo.__lista_articulos:
            print(articulo)
            contador += 1
        print("Cantidad total de articulo es:", contador)
        print("-"*50)
...
class Albaran:
    ...
    def __str__(self):
        return f"{self.get_num_entrada()} -> ID de Albaran: \
            {self.get_id_albaran()}, {self.get_articulo}, {self.get_cantidad}[uds]"

    def __repr__(self):
        return f"Albaran: {self.get_id_albaran()!r},{self.get_articulo()!r},{self.get_cantidad()!r}"
